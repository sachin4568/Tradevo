...
class AIContextBuilder:
    ...
    def build_dna_context(
        ...
        *,
        ...
        for t in transactions[:10]:
            _company_name = companies.get(t.company_id, type('O', (), {'name': t.company_id})()).name if t.company_id in companies else t.company_id
            trade_summary += (
    ...
    def build_portfolio_observations_context(
        ...
        *,
        ...
        for t in transactions[:10]:
            _company_name = companies.get(t.company_id, type('O', (), {'name': t.company_id})()).name if t.company_id in companies else t.company_id
            recent_trades += f"- {t.transaction_type} {int(t.quantity)} of {_company_name} at ₹{float(t.price):,.2f}\n"
