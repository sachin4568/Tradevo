...
export default function Portfolio() {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Charts Row: Allocation Donut + Performance Area Chart */}
      {!portfolio.isEmpty && (
      ...
      {/* AI Portfolio Review — optional section, graceful degradation */}
      <AISectionLoader
        isLoading={reviewLoading}
        isError={reviewError}
        ...
        {reviewData && (
          <div className="space-y-4">
            ...
            <AIInsightCard
              ...
              body={reviewData.characteristics}
              provenance={reviewData.provenance}
              category="portfolio"
            />
            ...
            {reviewData.sectorObservations.length > 0 && (
              <div className="rounded-xl border border-border bg-surface-1 p-4">
                <h3 className="mb-3 text-[13px] font-semibold text-tx-primary">
                  Sector Breakdown
                </h3>
                <div className="space-y-3">
                  {reviewData.sectorObservations.map((obs, i) => (
                    <div key={i} className="flex items-start gap-3">
                      <div className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-accent-subtle text-[11px] font-semibold text-accent">
                        {obs.allocationPercent}%
                      </div>
                      <div className="min-w-0 flex-1">
                        <p className="text-[13px] font-medium text-tx-primary">
                          {obs.sector}
                        </p>
                        <p className="mt-0.5 text-[12px] leading-relaxed text-tx-secondary">
                          {obs.description}
                        </p>
                      </div>
                    </div>
            ...
            {reviewData.educationalNote && (
              <div className="rounded-xl border border-border-subtle bg-surface-1/50 p-4">
                ...
                <div className="text-[12.5px] leading-relaxed text-tx-secondary">
                  <MarkdownContent content={reviewData.educationalNote} />
                </div>
              </div>
            )}
          </div>
      ...
      {/* AI Risk Summary — optional section, graceful degradation */}
      <AISectionLoader
        isLoading={riskLoading}
        isError={riskError}
        onRetry={handleRetryAI}
