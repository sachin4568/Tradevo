...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    - RATE_LIMIT_DEFAULT: default limit (e.g. '100/hour')
    - RATE_LIMIT_AUTH: limit for auth endpoints
    - RATE_LIMIT_AI: limit for AI endpoints
    - RATE_LIMIT_MARKET: limit for market data endpoints
    - RATE_LIMIT_TRADE: limit for trade execution endpoints
    """
    ...
    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint) -> Response:
        ...
        if request.url.path in ("/health", "/metrics", "/ready", "/live", "/api/v1/infra/status", "/api/v1/infra/version"):
            return await call_next(request)
