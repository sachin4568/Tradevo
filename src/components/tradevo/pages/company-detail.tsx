...
/* Generate 60-day chart data from the company's 20-point mini chart */
function generateChartData(miniChartData: readonly number[], basePrice: number) {
  ...
  // Seed a simple pseudo-random based on data values
  let seed = 0
  for (let i = 0; i < miniChartData.length; i++) {
    seed += miniChartData[i] * (i + 1)
  }
  const seededRandom = () => {
  ...
  for (let i = 0; i < 60; i++) {
    ...
    // Map the 60-day index back to the 20-point data with noise
    const t = i / 59
    const idx = t * (miniChartData.length - 1)
    const lo = Math.floor(idx)
    const hi = Math.min(lo + 1, miniChartData.length - 1)
    const frac = idx - lo
    const baseVal = miniChartData[lo]! * (1 - frac) + miniChartData[hi]! * frac
    const noise = (seededRandom() - 0.48) * range * 0.15
...
function ChartTooltip({ active, payload, label }: { active?: boolean; payload?: Array<{ value: number }>; label?: string }) {
  ...
  return (
    <div className="surface-card-static px-3 py-2 shadow-lg !border-border-subtle">
...
export function CompanyDetailPage() {
  ...
  const chartData = useMemo(
    () => (company ? generateChartData(company.miniChartData, company.price) : []),
    [company]
  )
                  ...
                  <AreaChart data={chartData} margin={{ top: 4, right: 4, bottom: 0, left: 4 }}>
                    <defs>
                      <linearGradient id="priceGrad" x1="0" y1="0" x2="0" y2="1">
                        ...
                        <stop offset="95%" stopColor="var(--tv-cyan)" stopOpacity="0.01" />
                      </linearGradient>
                    </defs>
                    <XAxis
                      dataKey="date"
