...
function generatePortfolioData() {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    value += (Math.random() - 0.42) * 8000
    benchmark += (Math.random() - 0.45) * 60
    value = Math.max(value, 1150000)
    benchmark = Math.max(benchmark, 9800)
...
export const mockPortfolioChart = generatePortfolioData()
